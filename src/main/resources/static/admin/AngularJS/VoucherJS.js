...
app.controller("voucher-ctrl", function ($scope, $http) {
        ...
        $http.get("http://localhost:8080/admin/voucher" + id).then(function (res) {
            ...
            //
            $scope.ngayBatDauSS = new Date(voucher.ngayBatDau);
...
//add
        $scope.create = function () {
            if ($scope.voucherAdd.loaiMucGiam === 'TIEN') {
                $scope.voucherAdd.mucGiamToiDa = 0;
            } else if ($scope.doiTuongSuDung == 1) {
                $scope.voucherAdd.soLuong = 0;
            }
...
//update
        $scope.update = function () {
            ...
            $http.put(urlWithId, voucher).then(function (resp) {
                alertify.success("Cập thành công");

                setTimeout(function () {
                    window.location.href = "http://localhost:8080/admin/voucher";
                }, 500);
            }).catch(err => {
                $scope.erMoTa = err.data.
                $scope.erMucGiam = err.data.mucGiam
                $scope.erMucGiamToiDa = err.data.mucGiamToiDa
                $scope.erLoaiMucGiam = err.data.loaiMucGiam
                $scope.erGiaTriDonHang = err.data.giaTriDonHang
                $scope.erNgayBatDau = err.data.ngayBatDau
                $scope.erNgayKetThuc = err.data.ngayKetThuc
                $scope.erSoLuong = err.data.soLuong
                $scope.erKhachHang = err.data.khachHang
                $scope.erdoiTuongSuDung = err.data.doiTuongSuDung
            })
        }
    }
)
